...
import { apiRoot, env, projectRoot, publicUrl } from "./config";

async function bootstrap() {
  const app = await NestFactory.create(AppModule);
  const host = env("HOST", "0.0.0.0");
  const port = Number(env("PORT", "4000"));
  const uploads = path.join(apiRoot(), "uploads");
  ...
  if (fs.existsSync(path.join(webOut, "index.html"))) {
    ...
    app.use((req: Request, res: Response, next: NextFunction) => {
      const apiPrefixes = ["/auth", "/users", "/posts", "/friends", "/chat", "/communities", "/health", "/uploads", "/socket.io"];
